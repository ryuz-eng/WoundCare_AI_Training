"""
Optimized Wound Classification Training Script
Goal: Lower loss while maintaining >=85% accuracy

Key Optimizations:
1. Reduced label smoothing (0.05 instead of 0.1)
2. Longer warmup + cosine annealing with restarts
3. Focal loss with adjusted gamma
4. Improved class balancing strategy
5. Monitor both loss AND accuracy for early stopping
...
def get_args():
    p = argparse.ArgumentParser(description="Optimized Wound Classification Training")
    
    # Required
    p.add_argument("--crop-root", required=True, help="Path to cropped images")
    p.add_argument("--save-dir", required=True, help="Directory to save outputs")
    ...
    p.add_argument("--epochs", type=int, default=200, help="Increased for better convergence")
    p.add_argument("--lr", type=float, default=2e-4, help="Slightly higher initial LR")
    p.add_argument("--min-lr", type=float, default=5e-7, help="Lower minimum LR")
    p.add_argument("--weight-decay", type=float, default=0.05)
    p.add_argument("--patience", type=int, default=35, help="Increased patience")
    p.add_argument("--warmup-epochs", type=int, default=5, help="Warmup epochs")
    
    # Loss optimization
    p.add_argument("--label-smooth", type=float, default=0.05, help="REDUCED from 0.1")
    p.add_argument("--mixup-alpha", type=float, default=0.3, help="Slightly reduced")
    p.add_argument("--cutmix-alpha", type=float, default=0.8, help="Slightly reduced")
    p.add_argument("--mixup-prob", type=float, default=0.4, help="Reduced probability")
    p.add_argument("--drop-path", type=float, default=0.2)
    p.add_argument("--use-focal", action="store_true", default=True)
    p.add_argument("--focal-gamma", type=float, default=1.5, help="REDUCED from 2.0")
    p.add_argument("--focal-alpha", type=float, default=0.25, help="Focal loss alpha")
    
    # Progressive training
    p.add_argument("--freeze-epochs", type=int, default=0, help="Freeze backbone for N epochs")
    p.add_argument("--gradual-unfreeze", action="store_true", help="Gradually unfreeze layers")
    
    # SWA
    p.add_argument("--swa", action="store_true", default=True)
    p.add_argument("--swa-start", type=int, default=120, help="Start later")
    p.add_argument("--swa-lr", type=float, default=5e-6)
    
    # TTA
    p.add_argument("--tta", action="store_true", default=True)
    p.add_argument("--tta-transforms", type=int, default=8, help="More TTA transforms")
    ...
    # Monitoring - IMPROVED
    p.add_argument("--monitor", type=str, default="balanced", 
                   choices=["loss", "acc", "balanced"],
                   help="What to monitor for best model (balanced = loss+acc)")
    p.add_argument("--min-delta", type=float, default=0.0001, help="Minimum improvement")
...
def get_tta_transforms(img_size, num_transforms=8):
...
class OptimizedFocalLoss(nn.Module):
    """Optimized Focal Loss with lower gamma for better loss values"""
    
    def __init__(self, alpha=None, gamma=1.5, label_smoothing=0.05, num_classes=4):
        super().__init__()
        self.gamma = gamma
...
@torch.no_grad()
def evaluate_with_tta(model, val_samples, img_size, device, num_tta=8):
    model.eval()
...
class WarmupCosineScheduler:
    ...
    def __init__(self, optimizer, warmup_epochs, max_epochs, min_lr, base_lr):
...
# ============================================================
# METRICS (Same plotting functions as before)
# ============================================================
...
def main():
    ...
    if args.use_focal:
        ...
        print(f"Loss: Optimized Focal Loss (gamma={args.focal_gamma}, alpha={args.focal_alpha}, smooth={args.label_smooth})")
    else:
    ...
    best_metric = float('inf') if args.monitor == "loss" else 0.0
    best_loss = float('inf')
    best_acc = 0.0
    best_probs = None
    best_labels = None
    ...
    csv_writer.writerow(['epoch', 'train_loss', 'val_loss', 'val_acc', 'lr', 'is_best'])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            use_cutmix = args.cutmix_alpha > 0 and np.random.random() < args.mixup_prob and not use_mixup
        ...
        # Determine if best based on monitoring strategy
        if args.monitor == "loss":
            current_metric = val_loss
            is_best = (current_metric < best_metric - args.min_delta)
        elif args.monitor == "acc":
            current_metric = val_acc
            is_best = (current_metric > best_metric + args.min_delta)
        else:  # balanced
            # Balanced: prioritize accuracy but also consider loss
            current_metric = val_acc - 0.1 * val_loss  # Weighted combination
            is_best = (current_metric > best_metric + args.min_delta)
        ...
        csv_writer.writerow([epoch, f"{train_loss:.6f}", f"{val_loss:.6f}", 
                            f"{val_acc:.6f}", f"{cur_lr:.8f}", "1" if is_best else "0"])
        csv_file.flush()
    ...
    if args.tta:
        ...
        if tta_acc > best_acc:
            print("TTA results are better! Using TTA predictions.")
    ...
    print(f"Monitor metric: {args.monitor} (min_delta={args.min_delta})")
    print(f"Best Val Loss: {best_loss:.4f} (epoch {np.argmin(val_loss_hist)+1})")
    print(f"Best Val Acc : {best_acc*100:.2f}% (epoch {np.argmax(val_acc_hist)+1})")
    print("=" * 70)
